...
// swiftlint:disable force_unwrapping attributes large_tuple closure_body_length trailing_closure
...
extension DataCache {
    ...
    public func write(data: Data, forKey key: String, alsoOnDisk: Bool = false) {
    ...
    public func write<T: Encodable>(codable: T, forKey key: String, alsoOnDisk: Bool = false) throws {
        let data = try JSONEncoder().encode(codable)
        write(data: data, forKey: key, alsoOnDisk: alsoOnDisk)
    }
    ...
    public func write(object: NSCoding, forKey key: String, alsoOnDisk: Bool = false) {
        ...
        write(data: dataToWrite, forKey: key, alsoOnDisk: alsoOnDisk)
    }

    /// Write a string for key
    public func write(string: String, forKey key: String, alsoOnDisk: Bool = false) {
        write(object: string as NSCoding, forKey: key, alsoOnDisk: alsoOnDisk)
    }

    /// Write a dictionary for key
    public func write(dictionary: [AnyHashable: Any], forKey key: String, alsoOnDisk: Bool = false) {
        write(object: dictionary as NSCoding, forKey: key, alsoOnDisk: alsoOnDisk)
    }

    /// Write an array for key
    public func write(array: [Any], forKey key: String, alsoOnDisk: Bool = false) {
        write(object: array as NSCoding, forKey: key, alsoOnDisk: alsoOnDisk)
    }
    ...
    public func readObject(forKey key: String) -> NSObject? {
        ...
        if let data = data {
            if #available(iOS 13, *) {
                if let loadObject = try? NSKeyedUnarchiver.unarchiveTopLevelObjectWithData(data) as? NSObject {
                    return loadObject
                }
            } else {
    ...
    public func readArray(forKey key: String) -> [Any]? {
        return readObject(forKey: key) as? [Any]
    }

    /// Read a dictionary for key
    public func readDictionary(forKey key: String) -> [AnyHashable: Any]? {
    ...
    public func write(image: UIImage, forKey key: String, format: ImageFormat? = nil, alsoOnDisk: Bool = false) {
        var data: Data?
